...
> {
  ...
  public render(): React.ReactElement<IReactMyGroupsProps> {
    ...
    return (
      <div
        className={styles.reactMyGroups}
      ...
      >
        ...
        {this.props.toggleSeeAll === false && !!this.props.seeAllLink && (
          <a aria-label={this.strings.seeAllLabel} href={this.props.seeAllLink}>
            <div className={styles.seeAll}>{this.strings.seeAll}</div>
          </a>
        )}
        <div className={styles.createComm}>
          <Icon iconName="Add" className={styles.addIcon} />
          <a href={this.props.createCommLink}>{this.strings.createComm}</a>
        </div>
              ...
              {this.props.layout === "Compact" ? (
                ...
                <ListLayout
                  ...
                  onRenderListItem={(
                    ...
                    finalSize: ISize,
                    isCompact: boolean
                  ) => this._onRenderListItem(item, finalSize, isCompact)}
                />
              )}
      ...
      this.setState((prevState) => ({
        groups: prevState.groups.map((group) =>
          group.id !== null
            ? {
                ...group,
                thumbnail: "data:image/jpeg;base64," + grouptbs[group.id],
                color: "#0078d4",
              }
            : group
        ),
          ...
          <div className={styles.gridContent}>
            ...
            <div className={styles.footerContent}>
              ...
              <div className={styles.footerColumn2}>
                <p aria-label={this.strings.created}>
                  <span
                    style={{
                      fontWeight: "700",
                      fontSize: "12px",
        ...
        <a className="community-list-item" href={item.url}>
          <div className={styles.cardBannerList}>
            <div className={styles.articleFlex} style={{ width: "60px" }}>
              <img
                className={styles.bannerImgList}
                src={item.thumbnail}
                alt={`${this.strings.altImgLogo} ${item.displayName}`}
              />
            </div>
